...
class MyTransformerLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, er_len=None):
        ...
        self.dropout3 = nn.Dropout(dropout)  # 新增Dropout层
        
        # Layer Norms
        self.norm1 = nn.LayerNorm(d_model)
        self.norm2 = nn.LayerNorm(d_model)
        self.norm3 = nn.LayerNorm(d_model)  # 新增Layer Norm
        
        # Linear layers
        self.linear1 = nn.Linear(d_model, dim_feedforward)
        self.linear2 = nn.Linear(dim_feedforward, d_model)
        self.linear3 = nn.Linear(d_model, dim_feedforward)  # 新增线性层
        self.linear4 = nn.Linear(dim_feedforward, d_model)  # 新增线性层
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        # 新增Feedforward
        src2 = self.linear4(self.dropout(F.relu(self.linear3(src))))
    ...
    # 自己写的
    # def forward(self, src, src_mask=None, src_key_padding_mask=None):
    # # def forward(self, src, attn_mask, src_key_padding_mask=None):
    #     src2 = self.self_attn(src, src, src, attn_mask=src_mask,
    #                           key_padding_mask=src_key_padding_mask)[0]
        
    #     src = src + self.dropout1(src2)
    #     src = self.norm1(src)
    #     src2 = self.dropout(F.relu(self.linear1(src)))
    #     # src2 = F.relu(self.linear2(src2))
...
class TransformerEncoderLayerSMA(Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1):
        super(TransformerEncoderLayerSMA,self).__init__()
        self.self_attn=StreamingMultiheadAttention(d_model, nhead, dropout=dropout)
        self.linear1 = Linear(d_model, dim_feedforward)
        self.dropout = Dropout(dropout)
        self.linear2 = Linear(dim_feedforward, d_model)
...
class TransformerDecoderLayer(Module):
    def __init__(self, d_model, nhead,dim_feedforward=2048,dropout=0.1,activation='relu',er_len=None) -> None:
        super(TransformerDecoderLayer,self).__init__()
        self.self_attn=MultiheadAttentionRPR(d_model,nhead,dropout=dropout,er_len=er_len)
        self.multihead_attn=MultiheadAttentionRPR(d_model,nhead,dropout=dropout)
...
class MultiheadAttentionRPR(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None, er_len=None):
        ...
        # self.in_proj_weight = Parameter(torch.empty(4 * embed_dim, embed_dim)) #这里为什么是3倍
        self.in_proj_weight = Parameter(torch.empty(3 * embed_dim, embed_dim)) #这里为什么是3倍
    ...
    def forward(self, query, key, value, key_padding_mask=None,
        ...
        if hasattr(self, '_qkv_same_embed_dim') and self._qkv_same_embed_dim is False:
            ...
            return simplified_rpr_attention(
                query, key, value, self.embed_dim, self.num_heads,
                self.in_proj_weight, self.in_proj_bias,
                self.bias_k, self.bias_v, self.add_zero_attn,
                self.dropout, self.out_proj.weight, self.out_proj.bias,
            ...
            # 原函数
            return multi_head_attention_forward_rpr( # in_proj_weight:[1536,512] out_proj.weight:[512,512]
                query, key, value, self.embed_dim, self.num_heads,
                self.in_proj_weight, self.in_proj_bias,
                self.bias_k, self.bias_v, self.add_zero_attn,
                self.dropout, self.out_proj.weight, self.out_proj.bias,
...
def multi_head_attention_forward_rpr(query,                       # type: Tensor
    ...
    qkv_same = torch.equal(query, key) and torch.equal(key, value) # true or false
    kv_same = torch.equal(key, value)

    tgt_len, bsz, embed_dim = query.size() #2048,4,512
    ...
    scaling = float(head_dim) ** -0.5 # 计算了 head_dim 的倒数的平方根

    if use_separate_proj_weight is not True:
        if qkv_same:
            # self-attention
            q, k, v = linear(query, in_proj_weight, in_proj_bias).chunk(3, dim=-1) #使用 linear(query, in_proj_weight, in_proj_bias) 对 query 进行线性变换。
    ...
    q = q.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1) #(32,2048,64) 
    if k is not None:
    ...
    attn_output_weights = torch.bmm(q, k.transpose(1, 2)) # qk相乘
...
def simplified_rpr_attention(query, key, value, er):
    """
    简化和加速的RPR注意力计算。
    使用固定大小的RPR矩阵和标准矩阵乘法。
    """
    # 假设er是固定大小的RPR矩阵，例如 [20, head_dim]
    # 这里的20可以是根据实验得出的最大相对位置差

    # 标准的QK^T计算
    attn_scores = torch.matmul(query, key.transpose(-2, -1))

    # 计算相对位置得分
    # 假设query的形状是 [batch_size, num_heads, seq_length, head_dim]
    seq_length = query.size(0)
    max_relative_position = er.size(0)
    
    # 生成相对位置索引
    relative_positions = torch.arange(seq_length).unsqueeze(0).to(query.device) - torch.arange(seq_length).unsqueeze(1).to(query.device)
    # 将相对位置限制在RPR矩阵的大小内
    clipped_relative_positions = torch.clamp(relative_positions, -max_relative_position, max_relative_position) + max_relative_position
    
    # 从RPR矩阵中获取相应的嵌入
    rpr_embeddings = er[clipped_relative_positions.view(-1)].view(seq_length, seq_length, -1)
    
    # 计算相对位置得分
    rpr_scores = torch.einsum('bhl,lld->bhl', query, rpr_embeddings.unsqueeze(2))
    
    # 将相对位置得分添加到原始注意力得分上
    attn_scores += rpr_scores.unsqueeze(1)  # 可能需要调整维度以匹配
